...
def draw_cylender2d(args, ax, p1, p2, s=1, y12=0, mu3=0):
    # p1: point_in
    # p2: point_out
    ...
    if rr > 0.93:
        ...
        while (end_distance(p1, p2) > l):
            ...
            xx = [p2.position[0], x]
            zz = [p2.position[2], z]
            color = caculate_color_layer(y, y12, mu3)
        ...
        y_p1 = p1.position[1]
        color = caculate_color_layer(y_p1, y12, mu3)
        ax.plot(xx, zz, color=(color, color, color), linewidth=p2.radius * 1.5)
...
def draw_segment2d_rotate(args, ax, line, angle, s, y12, mu3):
    ...
    for i in range(len(line2)):
        ...
        point_in = segment.point_in
        point_out = segment.point_out
        draw_cylender2d(args, ax, point_in, point_out, s=s, y12=y12, mu3=mu3)
...
def caculate_color_layer(y, y12, mu3):

    # Adding 13 to the y-value is to compensate for the negative offset 
    # caused by translating the 3D tree to the origin during rotation.
    x3 = y+13
    yy = 1 - np.exp(-1 * (y12 + mu3 * x3))
    return yy if yy <= 1 else 1
