...
def batch_process(proc_index, ranges, args, imagenet_images=None):
    ...
    for id_idx, i in enumerate(range(*ranges[proc_index])):
        ...
        for s in range(args.samples):
            ...
            if imagenet_images is not None:
                ...
                if BG_EN:
                    bg_im = np.random.normal(loc=0.0, size=(args.imsize, args.imsize, 3)) + np.random.uniform(
                        size=(1, 1, 3))
                    bg_im = (np.clip(bg_im, 0, 1) * 255).astype(np.uint8)
            ...
            if CHANGE_EN:
                curves1 = [bezier.Curve(n.T * args.imsize + np.random.uniform(-10, 10, size=n.T.shape), degree=2) for n in nodes1]
            ...
            lw1 = np.random.uniform(2.3, 2.7) * FWIDTH
            ...
            filename = join(args.output, '%.5d.png' % i)
