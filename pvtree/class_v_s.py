...
class Segment():
    def __init__(self, point_in, point_out):
        ...
        self.parent = None
    # def get_flow(self):
    #     flow = np.pi * self.length * self.radius * self.radius
    #     return flow
    # def get_press_drop(self):
    #     u = 1.6
    #     k = 8 * u / np.pi
    #     delta_p = k * self.length * self.get_flow() / np.power(self.radius, 4)
    #     return delta_p



# class TPoint():
#     def __init__(self, position, radius=0.5, end_point=None):
#
#         self.position = position
#         self.radius = radius
#         self.parent = None
