...
class FaceDatasetFolder(Dataset):
    def __init__(self, root_dir, local_rank):
        ...
        transform_8 = transforms.Compose([
            ...
            transforms.RandomErasing(p=0.8, scale=(0.01, 0.1), ratio=(0.5, 1.5), value=0.0),
            transforms.ToTensor(),
        ])
        # self.transform = transform_8
        # self.transform = transforms.Compose(
        #     [transforms.ToPILImage(),
        #      transforms.RandomPerspective(distortion_scale=0.1, p=0.8),
        #      transforms.Resize([140,140]),
...
class valDatasetFolder(Dataset):
    def __init__(self, root_dir, local_rank):
        super(valDatasetFolder, self).__init__()
        self.transform = transforms.Compose(
            [transforms.ToPILImage(),
